...
func TestLayer3FloatingIPPortForwardingsCreateDelete(t *testing.T) {
	client, err := clients.NewNetworkV2Client()
	th.AssertNoErr(t, err)
	...
	pf, err := CreatePortForwarding(t, client, fip.ID, port.ID)
	defer DeletePortForwarding(t, client, fip.ID, pf.ID)

	newPf, err := floatingips.GetPortForwarding(client, fip.ID, pf.ID).Extract()
	th.AssertNoErr(t, err)

	tools.PrintResource(t, newPf)

	allPages, err := floatingips.ListPortForwardings(client, floatingips.ListPortForwardingOpts{}, fip.ID).AllPages()
	th.AssertNoErr(t, err)

	allPFs, err := floatingips.ExtractPortForwardings(allPages)
	th.AssertNoErr(t, err)
