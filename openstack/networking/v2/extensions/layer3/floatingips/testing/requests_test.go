...
func TestPortForwardingList(t *testing.T) {
	...
	th.Mux.HandleFunc("/v2.0/floatingips/2f95fd2b-9f6a-4e8e-9e9a-2cbe286cbf9e/port_forwardings", func(w http.ResponseWriter, r *http.Request) {
		...
		fmt.Fprintf(w, ListPortForwardingsResponse)
	})

	count := 0

	floatingips.ListPortForwardings(fake.ServiceClient(), floatingips.ListPortForwardingOpts{}, "2f95fd2b-9f6a-4e8e-9e9a-2cbe286cbf9e").EachPage(func(page pagination.Page) (bool, error) {
		count++
		actual, err := floatingips.ExtractPortForwardings(page)
		if err != nil {
			t.Errorf("Failed to extract port forwardings: %v", err)
			return false, err
		}

		expected := []floatingips.PortForwarding{
			{
				Protocol:          "tcp",
		...
		th.TestJSONRequest(t, r, `
{
  ...
  "port_forwarding": {
	...
	options := floatingips.CreatePortForwardingOpts{
		Protocol:         "tcp",
	...
	pf, err := floatingips.CreatePortForwarding(fake.ServiceClient(), "2f95fd2b-9f6a-4e8e-9e9a-2cbe286cbf9e", options).ExtractPortForwarding()
	th.AssertNoErr(t, err)
...
func TestPortForwardingGet(t *testing.T) {
	th.SetupHTTP()
	defer th.TeardownHTTP()
		...
		fmt.Fprintf(w, `
{
  "port_forwarding": {
	...
	pf, err := floatingips.GetPortForwarding(fake.ServiceClient(), "2f245a7b-796b-4f26-9cf9-9e82d248fda7", "725ade3c-9760-4880-8080-8fc2dbab9acc").ExtractPortForwarding()
	th.AssertNoErr(t, err)
...
func TestDeletePortForwarding(t *testing.T) {
	...
	res := floatingips.DeletePortForwarding(fake.ServiceClient(), "2f245a7b-796b-4f26-9cf9-9e82d248fda7", "725ade3c-9760-4880-8080-8fc2dbab9acc")
	th.AssertNoErr(t, res.Err)
}
